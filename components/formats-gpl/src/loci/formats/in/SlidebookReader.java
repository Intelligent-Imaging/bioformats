...
public class SlidebookReader extends FormatReader {
  ...
  public SlidebookReader() {
    super("Slidebook", new String[] {"sld", "spl"});
  ...
  protected void initFile(String id) throws FormatException, IOException {
    ...
    // parse OME XML companion file, if any, for meta data
    boolean omeXMLFileValid = false;
    Location omeXMLFile = new Location(id + ".ome");
    if (omeXMLFile.exists()) {
      LOGGER.info("Using metadata in OME XML companion file.");

      OMEXMLServiceImpl omeXMLService = new OMEXMLServiceImpl();
      String xml = new Scanner(new File(omeXMLFile.getAbsolutePath())).useDelimiter("\\Z").next();

      if (omeXMLService.validateOMEXML(xml)) {
        try {
          OMEXMLMetadata omeXMLMetaData = omeXMLService.createOMEXMLMetadata(xml);
          this.setMetadataStore(omeXMLMetaData);
          ...
          for (int i = 0; i < getSeriesCount(); i++) {
            ...
            OMEXMLMetadata ome = (OMEXMLMetadata) core.get(i);

            long offset;
            for (int imageIndex = 0; imageIndex < ms.imageCount; imageIndex++) {
              offset = ome.getLongAnnotationValue(imageIndex);
              planeOffset[i][imageIndex] = offset;
            }
          }

          omeXMLFileValid = true;
        }
        ...
        LOGGER.info("Invalid metadata in OME XML companion file.");
      }
    }
